...
function cleanGame(input = {}) {
  return {
    ...
    rewards: Array.isArray(input.rewards) ? input.rewards.slice(0, 80).map(r => ({
    ...
    achievements: Array.isArray(input.achievements) ? input.achievements.map(a => cleanText(a, 80)).slice(0, 40) : [],
...
app.get('/api/dashboard', (req, res) => {
  ...
  const averageScore = games.length ? Math.round(games.reduce((n, g) => n + (g.score || 0), 0) / games.length) : 0;
...
app.listen(PORT, '0.0.0.0', () => console.log(`Cupid game running on ${PORT}`));
